...
$runner = Join-Path $repoRoot 'scripts\run_job_ready_recovery_v2_single.py'
$outputPath = Join-Path $repoRoot 'results\job_ready_recovery_v2\single_odys_r1'
...
if (Test-Path -LiteralPath $outputPath) {
    ...
    if ($entries.Count -gt 0) { throw "OUTPUT_COLLISION: $outputPath" }
}
...
if ($exitCode -eq 0) {
    Write-Output 'JOB_READY_V2_SINGLE_EXECUTION_COMPLETE'
