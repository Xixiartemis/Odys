"""Prepare or execute the three-run P4 Golden effectiveness probe.

The PowerShell wrapper supplies only paths and the human-owned credential
gate.  This file remains static Python so task IDs and runner semantics cannot
be altered by PowerShell interpolation.
"""
...
PROBE_TASK_IDS = (
    "CWR-06",  # local repair
    "CWR-01",  # affected-subgraph semantics
    "CWR-03",  # macro-replan semantics
)
PROBE_CONFIG_IDS = ("odys_p3",)
PROBE_REPEATS = (1,)
EXPECTED_RUNS = 3
...
def select_probe_runs(snapshot: ProtocolSnapshot):
    """Select exactly the three frozen manifest tasks for one Odys repeat."""
    selected = tuple(
        run
        for run in select_runs(snapshot, headline=True)
        if run.task["task_id"] in PROBE_TASK_IDS
        and run.config["config_id"] in PROBE_CONFIG_IDS
        and run.repeat_index in PROBE_REPEATS
    )
    if len(selected) != EXPECTED_RUNS:
        raise RuntimeError(f"PROBE_SELECTION_COUNT_MISMATCH:{len(selected)}")
    if {run.task["task_id"] for run in selected} != set(PROBE_TASK_IDS):
        raise RuntimeError("PROBE_TASK_ID_RESOLUTION_FAILED")
    return selected


def _preflight(repo_root: Path, output: Path) -> int:
    snapshot = _load_snapshot(repo_root)
    selected = select_probe_runs(snapshot)
    ...
    print("TASK_IDS=" + ",".join(PROBE_TASK_IDS))
    print("CONFIGS=" + ",".join(PROBE_CONFIG_IDS))
    print("REPEATS=1")
...
def _execute(repo_root: Path, output: Path) -> int:
    ...
    print("GOLDEN_PROBE_EXECUTION_COMPLETE")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if output.exists() and any(output.iterdir()):
        raise RuntimeError(f"GOLDEN_PROBE_OUTPUT_EXISTS:{output}")
