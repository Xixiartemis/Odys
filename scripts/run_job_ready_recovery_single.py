"""Execute the one live gate for job-ready-recovery-v1.

This file is intentionally static and contains no credentials.  It is not
run during offline tests; the PowerShell wrapper supplies the local paths and
the human-owned credential environment.
"""
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run one job-ready Odys recovery proof")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if len(runs) != 1 or runs[0].config["config_id"] != "odys_p3":
        raise SystemExit("JOB_READY_SELECTION_INVALID")
    ...
    print("JOB_READY_SINGLE_EXECUTION_COMPLETE")
