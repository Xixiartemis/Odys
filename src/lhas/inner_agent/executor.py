...
class InnerAgentExecutor:
    ...
    def __init__(self, backend: InnerAgentBackend, allowed_capabilities=None, allowed_side_effect_capabilities=None, db=None, max_turns=None): self.backend=backend; self.allowed_capabilities=allowed_capabilities; self.allowed_side_effect_capabilities=allowed_side_effect_capabilities; self.db=db; self.max_turns=max_turns
    async def execute(self, request: ExecutionRequest) -> ExecutionResult:
        ...
        if self.db:
            ...
            failure_items=[item for item in result.trace if item.get("event")=="TOOL_OBSERVATION_SUMMARY" and item.get("status")=="FAILURE"]
            EventStore(self.db).append(event_type,task_id=request.task_id,run_id=request.run_id,attempt_id=request.attempt_id,payload={"status":result.status.value,"attempt_number":request.attempt_number,"turn_count":result.turn_count,"tool_call_count":result.tool_call_count,"tool_failure_count":len(failure_items),"completion_claim":result.completion_claim,"error_type":result.error_type})
