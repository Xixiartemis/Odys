...
class ContextReconstructionService:
    ...
    def reconstruct(self, task: Task, run_id, attempt_id, attempt_number, failure_report=None, recovery_action=None):
        checkpoint=self.repo.latest_for_run(run_id); cursor=checkpoint.event_cursor if checkpoint else 0; all_events=[e for e in self.events.list_for_run(run_id) if e.event_type != EventType.CHECKPOINT_CREATED]; delta=[e for e in all_events if (e.id or 0)>cursor]; previous=checkpoint.working_state if checkpoint else WorkingState(task_id=task.id,run_id=run_id); state=self.projector.project(previous,delta)
        ...
        snapshot.metrics=metrics; return snapshot, metrics
