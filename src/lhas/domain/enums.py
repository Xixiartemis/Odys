"""Domain state enums (docs/02_DOMAIN_MODEL.md)."""

from enum import Enum


class TaskStatus(str, Enum):
    """Lifecycle of a Task. Phase A uses CREATED/RUNNING/COMPLETED/ESCALATED;
    VALIDATING/RECOVERING/BLOCKED are reserved for Phase B+."""

    CREATED = "CREATED"
    READY = "READY"
    RUNNING = "RUNNING"
    VALIDATING = "VALIDATING"
    RECOVERING = "RECOVERING"
    COMPLETED = "COMPLETED"
    FAILED = "FAILED"
    BLOCKED = "BLOCKED"
    BLOCKED_PROVIDER = "BLOCKED_PROVIDER"
    ESCALATED = "ESCALATED"
    CANCELLED = "CANCELLED"


class RunStatus(str, Enum):
    """Lifecycle of a Run (mirrors Task lifecycle at run granularity)."""

    CREATED = "CREATED"
    RUNNING = "RUNNING"
    COMPLETED = "COMPLETED"
    FAILED = "FAILED"
    ESCALATED = "ESCALATED"
    CANCELLED = "CANCELLED"
    BLOCKED_PROVIDER = "BLOCKED_PROVIDER"


class ExecutionHealth(str, Enum):
    """Runtime health projection kept separate from durable RunStatus."""

    ACTIVE = "ACTIVE"
    IDLE = "IDLE"
    STALLED = "STALLED"
    UNKNOWN = "UNKNOWN"


class AttemptStatus(str, Enum):
    """Lifecycle of a single Attempt inside a Run."""

    PENDING = "PENDING"
    RUNNING = "RUNNING"
    COMPLETED = "COMPLETED"
    FAILED = "FAILED"
    TIMED_OUT = "TIMED_OUT"
    CRASHED = "CRASHED"


class ExecutionStatus(str, Enum):
    """Terminal verdict of one executor invocation (docs/04_EXECUTOR_PROTOCOL.md)."""

    SUCCESS = "SUCCESS"
    FAILURE = "FAILURE"
    TIMEOUT = "TIMEOUT"
    CRASH = "CRASH"


class EventType(str, Enum):
    """Event catalog.

    Phase A emits the subset defined by docs/10_LOGGING_SPEC.md plus the
    attempt/run lifecycle events required by the Phase A state machine
    (ATTEMPT_*/RUN_*/RETRY_SCHEDULED — an explicit, documented extension).
    Validation/recovery/human-approval events are emitted from Phase B on.
    """

    # Task lifecycle
    TASK_CREATED = "TASK_CREATED"
    TASK_STARTED = "TASK_STARTED"
    TASK_COMPLETED = "TASK_COMPLETED"
    TASK_FAILED = "TASK_FAILED"
    TASK_ESCALATED = "TASK_ESCALATED"
    TASK_CANCELLED = "TASK_CANCELLED"

    # Run lifecycle
    RUN_CREATED = "RUN_CREATED"
    RUN_STARTED = "RUN_STARTED"
    RUN_COMPLETED = "RUN_COMPLETED"
    RUN_FAILED = "RUN_FAILED"
    RUN_ESCALATED = "RUN_ESCALATED"

    # Attempt lifecycle
    ATTEMPT_STARTED = "ATTEMPT_STARTED"
    ATTEMPT_COMPLETED = "ATTEMPT_COMPLETED"
    ATTEMPT_FAILED = "ATTEMPT_FAILED"
    ATTEMPT_TIMED_OUT = "ATTEMPT_TIMED_OUT"
    ATTEMPT_CRASHED = "ATTEMPT_CRASHED"

    # Context
    CONTEXT_BUILT = "CONTEXT_BUILT"

    # Executor
    EXECUTOR_STARTED = "EXECUTOR_STARTED"
    EXECUTOR_EVENT = "EXECUTOR_EVENT"
    EXECUTOR_COMPLETED = "EXECUTOR_COMPLETED"
    EXECUTOR_FAILED = "EXECUTOR_FAILED"
    EXECUTION_CANCELLED = "EXECUTION_CANCELLED"
    EXECUTION_DEADLINE_EXCEEDED = "EXECUTION_DEADLINE_EXCEEDED"

    # Recovery (Phase A: deterministic decision; Phase B: classifier-driven)
    RETRY_SCHEDULED = "RETRY_SCHEDULED"

    # Validation / recovery pipeline (Phase B)
    VALIDATION_STARTED = "VALIDATION_STARTED"
    VALIDATION_PASSED = "VALIDATION_PASSED"
    VALIDATION_FAILED = "VALIDATION_FAILED"
    FAILURE_CLASSIFIED = "FAILURE_CLASSIFIED"
    RECOVERY_DECIDED = "RECOVERY_DECIDED"
    RECOVERY_STARTED = "RECOVERY_STARTED"

    # Human approval gate (Phase F)
    HUMAN_APPROVAL_REQUIRED = "HUMAN_APPROVAL_REQUIRED"
    HUMAN_APPROVAL_GRANTED = "HUMAN_APPROVAL_GRANTED"
    ACTION_SUBMITTED = "ACTION_SUBMITTED"
    INNER_AGENT_STARTED = "INNER_AGENT_STARTED"
    INNER_AGENT_LLM_TURN_STARTED = "INNER_AGENT_LLM_TURN_STARTED"
    INNER_AGENT_LLM_TURN_COMPLETED = "INNER_AGENT_LLM_TURN_COMPLETED"
    INNER_AGENT_TOOL_STARTED = "INNER_AGENT_TOOL_STARTED"
    INNER_AGENT_TOOL_COMPLETED = "INNER_AGENT_TOOL_COMPLETED"
    INNER_AGENT_COMPLETED = "INNER_AGENT_COMPLETED"
    INNER_AGENT_FAILED = "INNER_AGENT_FAILED"
    WORKSPACE_EDIT_STARTED = "WORKSPACE_EDIT_STARTED"
    WORKSPACE_EDIT_COMPLETED = "WORKSPACE_EDIT_COMPLETED"
    WORKSPACE_EDIT_FAILED = "WORKSPACE_EDIT_FAILED"
    WORKSPACE_RESTORED = "WORKSPACE_RESTORED"
    INNER_AGENT_TOOL_OBSERVATION = "INNER_AGENT_TOOL_OBSERVATION"
    CHECKPOINT_CREATED = "CHECKPOINT_CREATED"
    CONTEXT_RECONSTRUCTION_STARTED = "CONTEXT_RECONSTRUCTION_STARTED"
    CONTEXT_RECONSTRUCTION_COMPLETED = "CONTEXT_RECONSTRUCTION_COMPLETED"
    CONTEXT_RECONSTRUCTION_FAILED = "CONTEXT_RECONSTRUCTION_FAILED"
    RUN_RESUME_STARTED = "RUN_RESUME_STARTED"
    RUN_RESUME_COMPLETED = "RUN_RESUME_COMPLETED"
    RUN_RESUME_FAILED = "RUN_RESUME_FAILED"
    WORKSPACE_RECOVERY_STATE = "WORKSPACE_RECOVERY_STATE"

    GOAL_CREATED = "GOAL_CREATED"
    PLAN_CREATED = "PLAN_CREATED"
    PLAN_STARTED = "PLAN_STARTED"
    PLAN_STEP_STARTED = "PLAN_STEP_STARTED"
    PLAN_STEP_COMPLETED = "PLAN_STEP_COMPLETED"
    PLAN_STEP_FAILED = "PLAN_STEP_FAILED"
    PLAN_STEP_READY = "PLAN_STEP_READY"
    PLAN_STEP_BLOCKED = "PLAN_STEP_BLOCKED"
    PLAN_COMPLETED = "PLAN_COMPLETED"
    PLAN_FAILED = "PLAN_FAILED"

    # Phase 3 — Typed TaskGraph transition provenance
    STEP_STATE_TRANSITION = "STEP_STATE_TRANSITION"
    STEP_PRECONDITION_FAILED = "STEP_PRECONDITION_FAILED"
    PLAN_STALE_REJECTED = "PLAN_STALE_REJECTED"
    TOOL_CALL_STARTED = "TOOL_CALL_STARTED"
    TOOL_CALL_COMPLETED = "TOOL_CALL_COMPLETED"
    TOOL_CALL_FAILED = "TOOL_CALL_FAILED"

    # Agent Platform Foundation
    ROOT_AGENT_STARTED = "ROOT_AGENT_STARTED"
    ROOT_AGENT_COMPLETED = "ROOT_AGENT_COMPLETED"
    PLAN_REQUESTED = "PLAN_REQUESTED"
    DELEGATION_CREATED = "DELEGATION_CREATED"
    DELEGATION_STARTED = "DELEGATION_STARTED"
    DELEGATION_COMPLETED = "DELEGATION_COMPLETED"
    DELEGATION_FAILED = "DELEGATION_FAILED"
    CHILD_RUN_LINKED = "CHILD_RUN_LINKED"
    SKILL_DISCOVERED = "SKILL_DISCOVERED"
    SKILL_LOADED = "SKILL_LOADED"
    MEMORY_READ = "MEMORY_READ"
    MEMORY_WRITE_REQUESTED = "MEMORY_WRITE_REQUESTED"
    MEMORY_WRITTEN = "MEMORY_WRITTEN"
    SESSION_SEARCHED = "SESSION_SEARCHED"
    KNOWLEDGE_SEARCHED = "KNOWLEDGE_SEARCHED"
    MCP_SERVER_CONNECTED = "MCP_SERVER_CONNECTED"
    MCP_TOOL_DISCOVERED = "MCP_TOOL_DISCOVERED"
    GOAL_COMPLETED = "GOAL_COMPLETED"
    FAILURE_ROUTED = "FAILURE_ROUTED"

    # Native Harness Core. Payloads are bounded projections; model reasoning
    # and raw credentials are never event data.
    NATIVE_EXECUTION_SNAPSHOT = "NATIVE_EXECUTION_SNAPSHOT"
    NATIVE_MODEL_TURN_STARTED = "NATIVE_MODEL_TURN_STARTED"
    NATIVE_MODEL_TURN_COMPLETED = "NATIVE_MODEL_TURN_COMPLETED"
    MODEL_CALL_STARTED = "MODEL_CALL_STARTED"
    MODEL_RESPONSE_RECEIVED = "MODEL_RESPONSE_RECEIVED"
    MODEL_RESPONSE_PARSED = "MODEL_RESPONSE_PARSED"
    MODEL_RESPONSE_REJECTED = "MODEL_RESPONSE_REJECTED"
    NATIVE_MODEL_RESPONSE_REJECTED = "NATIVE_MODEL_RESPONSE_REJECTED"
    NATIVE_TOOL_REQUESTED = "NATIVE_TOOL_REQUESTED"
    NATIVE_TOOL_STARTED = "NATIVE_TOOL_STARTED"
    NATIVE_TOOL_OBSERVED = "NATIVE_TOOL_OBSERVED"
    NATIVE_TOOL_RECONCILED = "NATIVE_TOOL_RECONCILED"
    COMPLETION_CANDIDATE_CREATED = "COMPLETION_CANDIDATE_CREATED"
    COMPLETION_CANDIDATE_ACCEPTED = "COMPLETION_CANDIDATE_ACCEPTED"
    COMPLETION_CANDIDATE_REJECTED = "COMPLETION_CANDIDATE_REJECTED"
    VALIDATION_FAILURE_CREATED = "VALIDATION_FAILURE_CREATED"
    REPLAN_SIGNAL_CREATED = "REPLAN_SIGNAL_CREATED"
    NATIVE_BUDGET_EXHAUSTED = "NATIVE_BUDGET_EXHAUSTED"
    DELEGATION_DELIVERY_PENDING = "DELEGATION_DELIVERY_PENDING"
    DELEGATION_DELIVERED = "DELEGATION_DELIVERED"
    DELEGATION_DELIVERY_CONSUMED = "DELEGATION_DELIVERY_CONSUMED"
    CHILD_OUTCOME_RECORDED = "CHILD_OUTCOME_RECORDED"
    RUNTIME_TARGET_SWITCH_REQUESTED = "RUNTIME_TARGET_SWITCH_REQUESTED"
    RUNTIME_TARGET_SWITCH_PENDING = "RUNTIME_TARGET_SWITCH_PENDING"
    RUNTIME_TARGET_SWITCH_COMMITTED = "RUNTIME_TARGET_SWITCH_COMMITTED"
    RUNTIME_TARGET_SWITCH_FAILED = "RUNTIME_TARGET_SWITCH_FAILED"
    NATIVE_PROVIDER_FAILURE = "NATIVE_PROVIDER_FAILURE"
    PROVIDER_HEALTH_CHANGED = "PROVIDER_HEALTH_CHANGED"
    REPLAN_ACCEPTED = "REPLAN_ACCEPTED"
    REPLAN_REJECTED = "REPLAN_REJECTED"
    STEP_FAILURE_PROVENANCE = "STEP_FAILURE_PROVENANCE"

    # Phase 3.3 — Repair execution
    REPAIR_STARTED = "REPAIR_STARTED"
    REPAIR_COMPLETED = "REPAIR_COMPLETED"
    REPAIR_STEP_INVALIDATED = "REPAIR_STEP_INVALIDATED"

    # Runtime side-effect receipt lifecycle.  These events record facts;
    # recovery authority remains outside the receipt projection.
    SIDE_EFFECT_REQUEST_CREATED = "SIDE_EFFECT_REQUEST_CREATED"
    SIDE_EFFECT_DISPATCH_STARTED = "SIDE_EFFECT_DISPATCH_STARTED"
    SIDE_EFFECT_COMMITTED = "SIDE_EFFECT_COMMITTED"
    SIDE_EFFECT_OBSERVATION_RECEIVED = "SIDE_EFFECT_OBSERVATION_RECEIVED"
    SIDE_EFFECT_RECONCILED = "SIDE_EFFECT_RECONCILED"
    SIDE_EFFECT_FAILED = "SIDE_EFFECT_FAILED"
    SIDE_EFFECT_COMMIT_UNKNOWN = "SIDE_EFFECT_COMMIT_UNKNOWN"


class FailureClass(str, Enum):
    """Failure taxonomy families (docs/07_FAILURE_TAXONOMY.md)."""

    EXECUTION = "EXECUTION"
    DATA = "DATA"
    REASONING = "REASONING"
    CONTEXT = "CONTEXT"
    ACTION = "ACTION"
    UNKNOWN = "UNKNOWN"


class FailureType(str, Enum):
    """Failure taxonomy types (docs/07_FAILURE_TAXONOMY.md)."""

    # EXECUTION
    TIMEOUT = "TIMEOUT"
    EXECUTOR_CRASH = "EXECUTOR_CRASH"
    TOOL_ERROR = "TOOL_ERROR"
    NETWORK_ERROR = "NETWORK_ERROR"
    QUOTA_EXHAUSTED = "QUOTA_EXHAUSTED"
    BILLING_OR_CREDIT_EXHAUSTED = "BILLING_OR_CREDIT_EXHAUSTED"
    AUTH_INVALID = "AUTH_INVALID"
    PROVIDER_UNAVAILABLE = "PROVIDER_UNAVAILABLE"
    PROVIDER_TIMEOUT = "PROVIDER_TIMEOUT"
    MALFORMED_PROVIDER_RESPONSE = "MALFORMED_PROVIDER_RESPONSE"
    UNKNOWN_PROVIDER_FAILURE = "UNKNOWN_PROVIDER_FAILURE"
    BUDGET_EXHAUSTED = "BUDGET_EXHAUSTED"
    # DATA
    EMPTY_RESULT = "EMPTY_RESULT"
    INVALID_JD = "INVALID_JD"
    DUPLICATE_JOB = "DUPLICATE_JOB"
    EXPIRED_JOB = "EXPIRED_JOB"
    MISSING_REQUIRED_FIELD = "MISSING_REQUIRED_FIELD"
    VERIFICATION_REJECTED = "VERIFICATION_REJECTED"
    # REASONING
    WRONG_MATCH = "WRONG_MATCH"
    WRONG_ASSUMPTION = "WRONG_ASSUMPTION"
    BAD_RANKING = "BAD_RANKING"
    INCOMPLETE_ANALYSIS = "INCOMPLETE_ANALYSIS"
    # CONTEXT
    MISSING_CONTEXT = "MISSING_CONTEXT"
    STALE_CONTEXT = "STALE_CONTEXT"
    CONTEXT_CONFLICT = "CONTEXT_CONFLICT"
    CONTEXT_OVERLOAD = "CONTEXT_OVERLOAD"
    # ACTION
    LOGIN_REQUIRED = "LOGIN_REQUIRED"
    FORM_CHANGED = "FORM_CHANGED"
    FIELD_VALIDATION_ERROR = "FIELD_VALIDATION_ERROR"
    UPLOAD_FAILED = "UPLOAD_FAILED"
    DUPLICATE_APPLICATION = "DUPLICATE_APPLICATION"
    APPROVAL_REQUIRED = "APPROVAL_REQUIRED"
    # FALLBACK
    UNKNOWN = "UNKNOWN"


class FailureLevel(str, Enum):
    """Routing boundary; dynamic plan/goal recovery remains out of scope."""

    TOOL = "TOOL"
    ATTEMPT = "ATTEMPT"
    TASK = "TASK"
    PLAN = "PLAN"
    GOAL = "GOAL"


class RecoveryActionType(str, Enum):
    """Recovery actions (docs/08_RECOVERY_POLICY.md V0 default policy)."""

    RETRY_WITH_FAILURE_CONTEXT = "RETRY_WITH_FAILURE_CONTEXT"
    RETRY_WITH_EXPANDED_CONTEXT = "RETRY_WITH_EXPANDED_CONTEXT"
    ESCALATE = "ESCALATE"
    ABORT = "ABORT"
    HUMAN_APPROVAL = "HUMAN_APPROVAL"
    BLOCK_PROVIDER = "BLOCK_PROVIDER"
