...
| Events/observability | L3 | `EventStore`, native snapshots/invocations, runtime side-effect receipts, JSONL traces | L3 | L2 | L3 | OH SDK `EventLog`; Pi session events | adopt shapes/exporters | P1 | NO | OH SDK `57f5cc9`; Pi `71dca871` | receipt lifecycle is runtime-owned; external unknown state fails closed |
...
| Cancellation | L3 | `ExecutionControlToken`; provider/tool/Safe CLI/MCP/recovery/child propagation; 14 offline adversarial tests | L3 | L2 | L3 | Pi AbortSignal; OH CancellationToken | Odys-owned root propagation | NONE | NO | Pi `71dca871`; OH SDK `57f5cc9` | local/bounded path closed; committed receipt survives cancellation |
...
| Repair/recovery | L3 | P45 recovery contract, shared budget, lineage, revalidation | L2 | L2 | L2 | no direct substitute | Odys-owned | NONE | YES | local only | receipt facts are available without moving recovery authority |
...
P0-A is closed only for the covered local/bounded path. P0-B is closed for
declared local/bounded adapters and the deterministic idempotent fake; unknown
external commit state remains fail-closed and is outside automatic replay.

## P0-B side-effect receipt scope

`SideEffectReceipt` and its append-only event lifecycle are runtime-owned and
independent of benchmark runners. The receipt binds Odys run, attempt, step,
and tool-call identity. The capability and ToolContract declarations expose
effect class, idempotency, receipt, reconciliation, and replay facts without
moving recovery authority into the receipt layer.

Local reversible/durable effects are supported with workspace digest
